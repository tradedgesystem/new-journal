...
This repository contains a Google Apps Script that instantly builds a Google Sheets workbook for backtesting the MES, M2K, MYM, and MCL futures contracts. The template supports one to two trades per day (or no trades) across up to 150 trading days, color-coded drop-downs for outcomes, automatic R-multiple calculations, and a summary dashboard with the key metrics you asked for.
...
### Customization tips
- Adjust `maxDays` or `tradesPerDay` in `setupTradesSheet_` to change how many pre-filled rows the template reserves (defaults to 150 days with two trades per day).
